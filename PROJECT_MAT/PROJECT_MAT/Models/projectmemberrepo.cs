...
namespace PROJECT_MAT.Models
{
...

    {
        ...
        public async Task<List<object>> GetAllmembers(int id)
        {
            try
            {
                var use = await _dbcontext.ProjectMembers.Join(_dbcontext.Users, c => c.UserId, cm => cm.UserId, (c, cm) => new {ProjectMemberId = c.ProjectMemberId,ProjectId = c.ProjectId, UserId = c.UserId,FirstName = cm.FirstName}).Where(x => x.ProjectId == id).ToListAsync<object>();
